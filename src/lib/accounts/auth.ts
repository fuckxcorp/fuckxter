...
export async function signIn(input: {
  ...
  if (!response.account) throw new ApiError("登录响应中没有账号信息。");
...
export async function updateProfile(input: {
  ...
  if (!response.account) throw new ApiError("资料响应中没有账号信息。");
  const next = normalizeAccount(response.account);
  setAccount(next);
  return next;
}
...
export async function confirmTwoFactor(code: string): Promise<Account> {
  ...
  if (!response.account) throw new ApiError("双重验证响应中没有账号信息。");
  const next = normalizeAccount(response.account);
  setAccount(next);
  return next;
}
