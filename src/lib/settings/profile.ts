...
import { getAccount, hydrateSession, updateProfile } from "../accounts/auth";
import { avatarGradient } from "../ui/dom";
import { ApiError, apiEndpoint } from "../core/http";
import { isValidBirthday, isValidUsername } from "../core/validation";
...
export function mountProfileSettings(
  ...
  const profileStatus = profileForm.querySelector<HTMLElement>(".form-status")!;
  ...
  const fillProfileForm = () => {
    ...
    if (account.headerUrl) {
      ...
      headerImage.className = "profile-header-preview";
  ...
  profileForm.addEventListener("submit", async (event) => {
    ...
    const gender =
      genderSelect.value === "自定义"
        ...
        : genderSelect.value;
    const button =
      profileForm.querySelector<HTMLButtonElement>(".primary-btn")!;
